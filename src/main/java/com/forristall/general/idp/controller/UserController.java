...
import com.forristall.general.idp.entity.RecoveryPhone;
...
import com.forristall.general.idp.repo.AccountVerificationRepo;
...
import com.forristall.general.idp.util.LoginData;
import com.forristall.general.idp.util.RestError;
import com.forristall.general.idp.util.SessionCookie;
      ...
      try {
        ...
        awsEmailService.sendMessage(
                awsEmailService.createSimpleMailMessage(
                        user.getEmail(),
                        "Forristall General IDP User Verification",
      ...
      if (user.getRecoveryEmail() != null) {
        ...
        awsEmailService.sendMessage(
                awsEmailService.createSimpleMailMessage(
                        ...
                        "Forristall General IDP Recovery Email Verification",
  ...
  String login(
    ...
    if (error.isPresent()) {
      ...
      if (users.size() != 1) {
        ...
        response.addCookie(new Cookie("ForristallGeneralIdp", URLEncoder.encode(cookie.toString(), StandardCharsets.UTF_8)));
        response.setStatus(HttpStatus.OK.value());
